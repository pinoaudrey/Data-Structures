...
#include "MyLinkedList_a175p516.h"
...
{ 
  private:
    ...
    HashFunc<KeyType> hf;       // the hash function object
    ...
    void preCalPrimes(const size_t n)
    {
        // code begins
        bool is_prime[n + 1];
        std::fill_n(is_prime, sizeof(is_prime), true);
    ...
    {
        // code begins
        auto & obj = hash_table[hf.univHash(key, hash_table.size())];
        auto itr = obj -> begin();
        for(; itr != obj -> end(); itr++){
            if((*itr).key == key){
                return itr;
